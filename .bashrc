...
git-prompt () {
	...
	if [ "$BRANCH" != "" ]; then
		...
		PS1="$WHITE[Bacarini \W]${NO_COLOR}(${PROMPT_COLOR}${BRANCH}${NO_COLOR}${STATE}) " # (${YELLOW}$(rvm_version)${NO_COLOR})\n$ "
	else
		PS1="$WHITE[Bacarini \W]${NO_COLOR} " # (${YELLOW}$(rvm_version)${NO_COLOR})\n\$ "
	fi
}
...
git-pair (){
	local default_name="Bacarini"
	local default_email="bruno.bacarini@locaweb.com.br"
